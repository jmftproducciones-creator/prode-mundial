...
function dedupeTournamentSubmissions(tournament) {
  ...
  tournament.submissions.forEach(submission => {
    ...
    byEmail.set(email, preferSubmissionForLeaderboard(byEmail.get(email), submission));
  });
...
function handleLeaderboard(req, res) {
  ...
  if (mtParticipants) {
    const existingEmails = new Set(leaderboard.map(row => normalizeEmail(row.player.email)));
    ...
    if (tenant) {
      ...
      if (tenantData && tenantData.users) {
        Object.values(tenantData.users).forEach(user => {
          const email = normalizeEmail(user.email);
          if (!existingEmails.has(email) && user.active !== false) {
            leaderboard.push({
