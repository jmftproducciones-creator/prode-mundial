  ...
  groupKeys().forEach(group => {
    ...
    groupFixtures(group).forEach(match => {
      ...
      const updateGroupTable = event => {
        ...
        syncGroupScoresFromDom(matches, model);
        matches.querySelector(".group-standings-body").innerHTML = standingsMarkup(model, group);
      };
...
function pruneDependentWinners(model, changedMatch) {
  ...
  const directDependents = ALL_BRACKET_MATCHES
    .filter(([, leftSlot, rightSlot]) => leftSlot === changedRef || rightSlot === changedRef)
    .map(([id]) => id);
  ...
  list.innerHTML = state.tournaments.map(tournament => `
    <article class="tournament-card">
      <div>
        ...
        <p>${tournament.isGlobal ? "Ranking global" : "Torneo privado"}</p>
      </div>
...
async function loadTournaments() {
  try {
    ...
    const data = await apiJson(`/api/tournaments?joined=${encodeURIComponent(joined)}`);
...
async function createTournament(name, code) {
...
async function joinTournamentByCode(code, name = "", showMessage = true) {
  ...
  const data = await apiJson("/api/join-tournament", {
    method: "POST",
    body: JSON.stringify({ name, code: normalized })
  });
  rememberTournamentCode(data.tournament.code);
  await loadTournaments();
...
document.getElementById("tournamentForm").addEventListener("submit", async event => {
  ...
  if (!name || !code) {
    status.hidden = false;
    status.textContent = "Completa nombre y clave del torneo.";
    return;
  }
  status.hidden = false;
  status.textContent = "Creando torneo...";
  try {
    const tournament = await createTournament(name, code);
    const link = `${window.location.origin}${invitePathFor(tournament)}`;
    input.value = "";
    codeInput.value = "";
    status.innerHTML = `
