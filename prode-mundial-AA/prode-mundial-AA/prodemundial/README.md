...
Para usar el fixture del Clausura necesitas configurar la API key de API-Football:

```bash
APIFOOTBALL_KEY=tu_api_key npm start
```

Tambien podes copiar `config.example.json` como `config.json` para uso local. No subas `config.json` a Git.
...
- `PORT`: puerto interno del servidor. Por defecto `3000`.
- `APIFOOTBALL_KEY`: API key de API-Football para el Torneo Clausura.
- `DATA_DIR`: carpeta donde se guardan `prode-store.json` y `live-results.json`. En VPS conviene usar `/var/lib/prode-mundial`.
- `CLAUSURA_ACTIVE_ROUND`: fecha abierta para predecir. Por ejemplo `17`.
- `LIVE_RESULTS_PROVIDER`: `local`, `worldcupapi` o `thesportsdb`.

Guia completa: `VPS.md`.
