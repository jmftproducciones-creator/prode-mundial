...
const LIVE_RESULTS_FILE = path.join(DATA_DIR, "live-results.json");
const STORE_FILE = path.join(DATA_DIR, "prode-store.json");
const CLAUSURA_ACTIVE_ROUND = Number(process.env.CLAUSURA_ACTIVE_ROUND || 17);
...
// Caché para no gastar la cuota diaria de API-Football
let cachedClausuraFixture = null;
let cachedClausuraTime = 0;
const CLAUSURA_FIXTURE_CACHE_MS = 10 * 60 * 1000;
...
function publicTournament(tournament) {
  return {
    ...
    invitePath: `/join/${encodeURIComponent(tournament.code)}`,
...
async function handleClausuraFixture(req, res) {
  // Caché en memoria por 24 horas
  if (cachedClausuraFixture && (Date.now() - cachedClausuraTime < CLAUSURA_FIXTURE_CACHE_MS)) {
    send(res, 200, JSON.stringify(cachedClausuraFixture));
    return;
  }
  ...
  if (!key) {
    try {
      ...
      key = configData.APIFOOTBALL_KEY;
    } catch (error) {
      console.log("No se encontró config.json, intentando continuar...");
    }
  }

  if (!key) {
    send(res, 500, JSON.stringify({ error: "Falta configurar APIFOOTBALL_KEY como variable de entorno o en config.json" }));
    return;
  }

  try {
    // Usamos 2024 temporalmente para que veas datos reales hasta que se sortee el 2026
    const url = "https://v3.football.api-sports.io/fixtures?league=128&season=2024";
    const response = await fetch(url, {
      headers: { "x-apisports-key": key }
    });

    if (!response.ok) throw new Error("Error al consultar API-Football");

    const data = await response.json();
    if (!data.response || data.response.length === 0) {
      send(res, 404, JSON.stringify({ error: "No hay partidos disponibles aún." }));
      return;
    }

    const fixturesGrouped = {};
    const teams = new Set();
    const badges = {};

    data.response.forEach(item => {
      const roundStr = String(item.league.round); // Ej: "Liga Profesional Argentina - 14"

      // EL FIX: Buscamos específicamente el número que está al final del string (\d+$)
      const roundMatch = roundStr.match(/\d+$/);
      const roundNumber = roundMatch ? Number(roundMatch[0]) : null;
      // Si encuentra un número al final lo usa, sino, limpia el nombre que traiga
      const matchRound = roundNumber ? `fecha${roundNumber}` : `fecha_${roundStr.replace(/\s+/g, '')}`;

      if (!fixturesGrouped[matchRound]) fixturesGrouped[matchRound] = [];
      ...
      teams.add(homeTeam);
      teams.add(awayTeam);
      badges[homeTeam] = item.teams.home.logo;
      badges[awayTeam] = item.teams.away.logo;

      fixturesGrouped[matchRound].push({
        id: `c_f${matchRound.replace('fecha', '')}_${item.fixture.id}`,
        home: homeTeam,
        away: awayTeam,
        roundNumber,
    ...
    Object.values(fixturesGrouped).forEach(matches => {
      matches.sort((a, b) => new Date(a.date || 0) - new Date(b.date || 0));
    });

    cachedClausuraFixture = {
      teams: Array.from(teams).sort(),
      badges: badges,
      activeRound: CLAUSURA_ACTIVE_ROUND,
      fixtures: fixturesGrouped
    };
    cachedClausuraTime = Date.now();
...
function handleTournaments(req, res) {
  ...
  const tournaments = store.tournaments
    .filter(tournament => tournament.isGlobal || joinedCodes.includes(tournament.code))
...
async function handleJoinTournament(req, res) {
  try {
    ...
    send(res, 200, JSON.stringify({ tournament: publicTournament(tournament) }));
  } catch (error) {
    send(res, 500, JSON.stringify({ error: error.message }));
  }
}
...
async function handleSubmitProde(req, res) {
  try {
    ...
    const competition = payload.tournament.competition || "mundial2026";
    const existingIndex = tournament.submissions.findIndex(item =>
      String(item.player?.email || "").toLowerCase() === email &&
      (item.prediction?.competition || "mundial2026") === competition
    );
...
function handleSubmission(req, res) {
  ...
  const competition = String(url.searchParams.get("competition") || "mundial2026");
  if (!email) {
    send(res, 400, JSON.stringify({ error: "Missing email" }));
    return;
  }
  ...
  const submission = [...(tournament.submissions || [])]
    .reverse()
    .find(item =>
      String(item.player?.email || "").toLowerCase() === email &&
      (item.prediction?.competition || "mundial2026") === competition
  ...
  if (!submission) {
    send(res, 404, JSON.stringify({ error: "Submission not found" }));
    return;
  }

  send(res, 200, JSON.stringify({ submission }));
}
...
function serveStatic(req, res) {
  ...
  const requested = req.url === "/" || cleanUrl.startsWith("/join/") ? "/index.html" : cleanUrl;
...
http.createServer((req, res) => {
  ...
  if (req.method === "GET" && req.url === "/api/clausura-fixture") {
    handleClausuraFixture(req, res);
    return;
  }
  ...
  if (req.method === "GET" && req.url.startsWith("/api/submission")) {
