...
function mergeDailyPlayEntry(current = null, incoming = null) {
  if (!current) return incoming || null;
  if (!incoming) return current;
  if (incoming.completed && !current.completed) return incoming;
...
function mergedUserGamePlays(store, email = "", tenantId = "") {
  const normalized = normalizeEmail(email);
  if (!normalized) return {};
  const merged = {};
  const sources = [store.globalGamePlays];
  if (tenantId) {
    sources.push(tenantStore(store, tenantId).gamePlays);
  } else {
    Object.values(store.tenants || {}).forEach(tenantData => sources.push(tenantData?.gamePlays));
  }
  sources.forEach(source => {
    const byDate = source?.[normalized];
    if (!byDate || typeof byDate !== "object") return;
    Object.entries(byDate).forEach(([date, day]) => {
      if (!merged[date]) merged[date] = {};
      merged[date].camisetadle = mergeDailyPlayEntry(merged[date].camisetadle, day?.camisetadle || null);
      merged[date].desafio = mergeDailyPlayEntry(merged[date].desafio, day?.desafio || null);
    });
  });
  return merged;
}

function currentDailyGamePlaysMerged(store, email = "", games = {}, tenantId = "") {
  const playsByDate = mergedUserGamePlays(store, email, tenantId);
  const plays = playsByDate?.[todayKey()] || {};
  const camisetadle = todayGameItem(games, "camisetadle");
  const desafio = todayGameItem(games, "desafio");
  return {
    camisetadle: plays.camisetadle?.challengeId === camisetadle?.id ? plays.camisetadle : null,
    desafio: plays.desafio?.challengeId === desafio?.id ? plays.desafio : null
  };
}
...
function totalFanPointsMerged(store, email = "", tenantId = "") {
  const byDate = mergedUserGamePlays(store, email, tenantId);
  return Object.values(byDate).reduce((sum, day) => {
    const camisetadle = Number(day?.camisetadle?.points || 0);
    const desafio = Number(day?.desafio?.points || 0);
    return sum + camisetadle + desafio;
...
async function handleCompanyLogin(req, res) {
  try {
    ...
    send(res, 200, JSON.stringify({
      ...
      dailyGamePlays: currentDailyGamePlaysMerged(store, email, publicGames(tenantData), tenant.id),
      tenant: publicTenant(tenant, tenantData)
    }));
  } catch (error) {
    send(res, 500, JSON.stringify({ error: error.message }));
...
async function handleProfileStats(req, res) {
  try {
    ...
    const points = totalFanPointsMerged(store, user.email, tenant?.id || "");
...
function handleCompanySession(req, res) {
  ...
  send(res, 200, JSON.stringify({
    ok: true,
    user,
    dailyGamePlays: currentDailyGamePlaysMerged(store, user.email, publicGames(tenantData), tenant.id),
    tenant: publicTenant(tenant, tenantData)
  }));
}
...
function handleGlobalGames(req, res) {
  ...
  send(res, 200, JSON.stringify({
    games,
    dailyGamePlays: user ? currentDailyGamePlaysMerged(store, user.email, games) : {}
  }));
}
...
async function handleDailyGamePlay(req, res) {
  try {
    ...
    const email = normalizeEmail(user.email);
    const date = todayKey();
    const primaryGamePlays = tenantData ? tenantData.gamePlays : store.globalGamePlays;
    if (!primaryGamePlays[email]) primaryGamePlays[email] = {};
    if (!primaryGamePlays[email][date]) primaryGamePlays[email][date] = {};
    const previous = primaryGamePlays[email][date][gameType];
    ...
    primaryGamePlays[email][date][gameType] = play;
    if (!store.globalGamePlays[email]) store.globalGamePlays[email] = {};
    if (!store.globalGamePlays[email][date]) store.globalGamePlays[email][date] = {};
    store.globalGamePlays[email][date][gameType] = play;
    if (tenantData) {
      if (!tenantData.gamePlays[email]) tenantData.gamePlays[email] = {};
      if (!tenantData.gamePlays[email][date]) tenantData.gamePlays[email][date] = {};
      tenantData.gamePlays[email][date][gameType] = play;
    }
    writeStore(store);
    send(res, 200, JSON.stringify({ ok: true, correct, play, dailyGamePlays: currentDailyGamePlaysMerged(store, email, games, tenant?.id || "") }));
  } catch (error) {
    send(res, 500, JSON.stringify({ error: error.message }));
  }
}
